...
func aula(u usuari) { // Sobre 'u' vull preguntar el tipus de dada que és: alumne o matriculat.
	switch u.(type) {
	case alumne: //Si és tipus alumne vull que diguis:
		fmt.Println("Estic dins de l'aula", u.(alumne).nom)
	case matriculat: //Si és tipus matriculat vull que diguis:
